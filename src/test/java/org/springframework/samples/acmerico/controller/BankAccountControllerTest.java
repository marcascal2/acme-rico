...
public class BankAccountControllerTest{
    ...
    void testInitCreationForm() throws Exception{
        mockMvc.perform(get("/accounts/{clientId}/new", TEST_CLIENT_ID))
        .andExpect(status().isOk())
        .andExpect(model().attributeExists("bankAccount"))
        .andExpect(model().attributeDoesNotExist("alias"))
        .andExpect(status().is2xxSuccessful())
        .andExpect(view().name("accounts/createAccountForm"));
     }
     ...
     void testProcessCreationFormWithErrors() throws Exception{
       // ModelAndView modelAndView = Mockito.mock(ModelAndView.class);
        ...
        mockMvc.perform(post("/accounts/{clientId}/new", TEST_CLIENT_ID)
        .param("accountNumber", TEST_BANK_ACCOUNT_NUMBER)
        .param("amount", "10000.")
        .param("alias", "Cuenta Personal"))
    ...
    void testShowClientById() throws Exception{
        ...
        .andExpect(view().name("accounts/showAccountInfo"))
        .andExpect(status().is2xxSuccessful());
    ...
    void testInitDeposit() throws Exception{
        ...
        .andExpect(model().attribute("bankAccount", hasProperty("client",is(javier))))
        .andExpect(status().is2xxSuccessful())
        .andExpect(view().name("accounts/depositMoney"));
