...
public class EmployeeControllerTest {
	...
	void testProcessCreationForm() throws Exception {
		mockMvc.perform(post("/employees/new").param("firstName", "Jose").param("lastName", "Garcia Dorado")
				.with(csrf()).param("salary", "2000.0")).andExpect(status().is2xxSuccessful());
	}

	@WithMockUser(value = "spring")
	@Test
	void testProcessCreationFormHasErrors() throws Exception {
		mockMvc.perform(post("/employees/new").param("firstName", "worker1")).andExpect(status().is4xxClientError());
	}

	@WithMockUser(value = "spring")
	@Test
	void testinitFindForm() throws Exception {
		mockMvc.perform(get("/employees/find")).andExpect(status().isOk())
				.andExpect(model().attributeExists("employee")).andExpect(view().name("employees/findEmployees"))
	...
	void testProcessFindFormHasEmployee() throws Exception {
		...
		mockMvc.perform(get("/employees").with(csrf()).param("lastName", employee.getLastName()))
				.andExpect(status().isOk()).andExpect(model().attributeExists("selections"))
				.andExpect(view().name("employees/employeesList")).andExpect(status().is2xxSuccessful());
	}
	...
	void testInitUpdateEmployeeForm() throws Exception {
		...
		mockMvc.perform(get("/employees/{employeeId}/edit", employee.getId())).andExpect(status().isOk())
	...
	void testProcessUpdateEmployeeForm() throws Exception {
		mockMvc.perform(post("/employees/{employeeId}/edit", employee.getId()).with(csrf()).param("firstName", "Jose")
				.param("lastName", "Garcia Dorado").param("salary", "1500.0").param("user.username", "user")
				.param("user.password", "pass")).andExpect(status().isFound())
	...
	void testShowEmployee() throws Exception {
		...
		mockMvc.perform(get("/employees/{employeeId}", employee.getId())).andExpect(status().isOk())
				.andExpect(model().attributeExists("employee"))
				.andExpect(model().attribute("employee", hasProperty("firstName", is("Jose"))))
				.andExpect(model().attribute("employee", hasProperty("lastName", is("Garcia Dorado"))))
				.andExpect(model().attribute("employee", hasProperty("salary", is(2000.0))))
	...
	void testProcessInitPersonalDataForm() throws Exception {
		...
		mockMvc.perform(get("/personalDataEmployee/{name}", user.getUsername())).andExpect(status().isOk())
				.andExpect(model().attributeExists("employee"))
				.andExpect(model().attribute("employee", hasProperty("firstName", is("Jose"))))
				.andExpect(model().attribute("employee", hasProperty("lastName", is("Garcia Dorado"))))
				.andExpect(model().attribute("employee", hasProperty("salary", is(2000.0))))
	...
	void testInitUpdatePersonalDataForm() throws Exception {
		...
		mockMvc.perform(get("/personalDataEmployee/{employeeId}/edit", employee.getId())).andExpect(status().isOk())
				.andExpect(model().attributeExists("employee"))
				.andExpect(model().attribute("employee", hasProperty("firstName", is("Jose"))))
				.andExpect(model().attribute("employee", hasProperty("lastName", is("Garcia Dorado"))))
				.andExpect(model().attribute("employee", hasProperty("salary", is(2000.0))))
	...
	void testProcessUpdatePersonalDataForm() throws Exception {
		mockMvc.perform(post("/personalDataEmployee/{employeeId}/edit", employee.getId()).with(csrf())
				.param("firstName", "Jose").param("lastName", "Garcia Dorado").param("salary", "1500.0")
				.param("user.username", "user").param("user.password", "pass")).andExpect(status().isFound())
	...
	void testProcessUpdatePersonalDataFormHasErrors() throws Exception {
		mockMvc.perform(post("/personalDataEmployee/{employeeId}/edit", employee.getId()).with(csrf())
				.param("firstName", "Jose").param("lastName", "Garcia Dorado")).andExpect(status().isOk())
				.andExpect(model().attributeHasErrors("employee"))
				.andExpect(model().attributeHasFieldErrors("employee", "salary"))
				.andExpect(view().name("employees/createOrUpdateEmployeeForm")).andExpect(status().is2xxSuccessful());
	}
	...
	void testDeleteEmployee() throws Exception {
		mockMvc.perform(get("/employees/{employeeId}/delete", employee.getId())).andExpect(status().isFound())
