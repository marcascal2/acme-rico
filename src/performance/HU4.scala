...
class HU4 extends Simulation {
	...
	object FindEmployeesForm {
		val findEmployeesForm = exec(http("FindEmployees")
			.get("/employees/find")
			.headers(headers_0))
		.pause(2)
	}

	object ShowEmployees {
		val showEmployees = exec(http("ShowEmployees")
			.get("/employees?lastName=")
			.headers(headers_0))
		.pause(2)
	}
		
	val scn = scenario("HU4").exec(Home.home,
			Login.login,
			FindEmployeesForm.findEmployeesForm,
